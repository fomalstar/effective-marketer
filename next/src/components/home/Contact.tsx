...
export default function Contact({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
      ...
      setErrorMessage(
        "The contact form is temporarily unavailable. Please email contact@effectivemarketer.com directly."
      );
      return;
    }
  ...
  if (renderFormOnly) {
    return (
      <>
        {formSection}
        {structuredDataScript.map((data, index) => (
          <script
            key={index}
          ...
          {showInfo ? (
            <div className="space-y-8">
              <div>
                ...
                <p className="text-gray-600 text-xl mb-8">{infoDescription}</p>
              </div>
