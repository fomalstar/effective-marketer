...
const services = [
  {
    name: "Google Autosuggests",
    href: "/google-autosuggest-ranking",
    icon: Search,
    desc: "Dominate search suggestions before results appear.",
  },
  {
    name: "AI SEO",
    href: "/ai-seo",
    icon: Brain,
    desc: "Get cited by ChatGPT, Gemini, and AI Overviews.",
  },
  {
    name: "AI Topical Map",
    href: "/ai-topical-map",
    icon: Map,
    desc: "Entity-rich blueprints built for Generative Intent.",
  },
  {
    name: "AI Lead Gen",
    href: "/lead-gen-ai-automation",
    icon: Mail,
    desc: "Automated multi-channel lead generation with AI.",
  },
];
...
export default function Header() {
  ...
  const openWithDelay = () => {
    ...
    hoverTimerRef.current = setTimeout(() => setIsServicesOpen(true), 100);
  };

  const closeWithDelay = () => {
    if (hoverTimerRef.current) clearTimeout(hoverTimerRef.current);
    hoverTimerRef.current = setTimeout(() => setIsServicesOpen(false), 180);
  };
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center space-x-9">
            ...
            >
              ...
              {isServicesOpen && (
                <div
                  className="absolute left-1/2 -translate-x-1/2 mt-3 w-[960px] bg-white/95 backdrop-blur-md border border-gray-100 shadow-2xl rounded-2xl p-8"
                  onMouseEnter={openWithDelay}
                  onMouseLeave={closeWithDelay}
                >
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {services.map((svc) => (
                      ...
                      >
                        ...
                        <div className="font-display text-[15px] leading-tight text-gray-900 font-semibold">
                          {svc.name}
                        </div>
                        <div className="text-[13px] leading-snug text-gray-600 mt-1">
                          {svc.desc}
                        </div>
                        <div className="text-primary-600 text-xs font-medium mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          Learn more →
                        </div>
                      </Link>
                    ))}
                  ...
                  <div className="mt-7 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Link
                      ...
                      target="_blank"
                      className="col-span-1 md:col-span-2 hidden md:block rounded-xl border border-gray-100 p-5 bg-white hover:bg-gray-50 transition-colors"
                    >
                      <div className="text-sm font-semibold text-gray-900 mb-1">
                        Professional SEO Dashboard
                      </div>
                      <p className="text-sm text-gray-600">
                        Track everything in one place with our AI SEO Dashboard — autosuggest rankings, topical map, AI SEO and more.
                      </p>
                      <div className="text-primary-600 text-xs font-medium mt-3">Open dashboard →</div>
                    </Link>
                    <Link
                      href="https://calendly.com/effectivemarketer/demo"
                      className="rounded-xl border border-primary-200 p-5 bg-gradient-to-r from-primary-50 to-secondary-50 hover:from-primary-100 hover:to-secondary-100 transition-colors"
                    >
        ...
        <div className="md:hidden py-4 border-t">
          <nav className="space-y-2">
            ...
            <Link
              href="https://calendly.com/effectivemarketer/demo"
              target="_blank"
              className="bg-gradient-to-r from-primary-500 to-secondary-500 text-white px-6 py-2 rounded-lg font-semibold hover:from-primary-600 hover:to-secondary-600 transition-all duration-300 shadow-md hover:shadow-lg mt-4 inline-block"
              onClick={closeMenus}
            >
              Book a Demo
            </Link>
